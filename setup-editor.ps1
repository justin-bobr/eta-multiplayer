# =============================================================================
#  Downloads latest Godot Mono Windows-Editor und entpackt nach Engine\.
#
#  Triggered by setup-editor.cmd wrapper. Direkt aufrufbar mit:
#    powershell -ExecutionPolicy Bypass -File setup-editor.ps1
# =============================================================================
...
# Nur Mono/C#-Build holen - der Standard-Build hat kein .NET runtime und keta.dll
# wuerde nicht laden. Pattern matched z.B. "Godot_v4.6.3-stable_mono_win64.zip".
...
# -----------------------------------------------------------------------------
# Stock export templates (Windows + Linux + Mac + Web + mobile) from the same
# release. The build-{platform}-{role}.cmd scripts use --export-release which
# expects these templates at:
#   %APPDATA%\Godot\export_templates\<version>.stable.mono\
# Without this folder, exporting to Linux from Windows fails.
# -----------------------------------------------------------------------------
...
# Version slug from tag, e.g. "4.6.3-stable" -> "4.6.3.stable.mono"
$tag = $rel.tag_name
...
# .tpz IS a zip - rename for Expand-Archive
...
# .tpz extracts to a 'templates/' subfolder
