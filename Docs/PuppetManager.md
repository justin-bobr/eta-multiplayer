...
| `Init(Node3D, NetClient)` | Binds the manager to a container and a NetClient and replays any pre-init known players. |
| `OnPlayerJoined(InitialPlayerState)` | Creates a puppet for a newly joined remote player. |
| `OnPlayerLeft(byte, LeaveReason)` | Removes the puppet for a player that has left. |
