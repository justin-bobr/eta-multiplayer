...
| `LodTierUpdateHz(PuppetPlayer.PuppetLodTier)` | Maps an LOD tier to its animation update rate (Hz); Off returns 0 to skip the advance entirely. |
...
| `PushSnapshot(uint, SnapshotPlayer)` | Pushes a new snapshot into the interp buffer and records its arrival wallclock. Out-of-order packets are dropped. If the gap since the last snapshot exceeds `ResumeGapUsec`, the buffer is wiped and the position snapped (FoW resume), avoiding a "slide through walls" lerp on PVS re-entry. |
| `ResetOnVisibilityResume(SnapshotPlayer)` | Called on a snapshot after a long visibility gap: clears the buffer so the next tick brackets only fresh data, snaps the position, and reseeds the smoothed interp delay. |
...
| `UpdateNameAndGlow(SnapshotPlayer)` | Pushes HP/Team/TeamSlot into the nameplate and body-ID material. Glow + label only for teammates (or all when spectating) and not Deathmatch. Fields are delta-checked to avoid per-frame churn. |
