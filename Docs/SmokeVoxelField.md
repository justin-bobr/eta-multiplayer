...
| `MethodName.AnyBlocksSight` | Cached name for the 'AnyBlocksSight' method. |
| `MethodName.Bake` | Cached name for the 'Bake' method. |
| `MethodName.BlocksSight` | Cached name for the 'BlocksSight' method. |
...
| `MethodName.SampleDensity` | Cached name for the 'SampleDensity' method. |
...
| Name | Summary |
|------|---------|
| `AnyBlocksSight(Vector3, Vector3)` | True if any active cloud blocks line of sight a->b. Deterministic (samples the sim grid only). |
| `Bake()` | Copies the density grid into per-slice images, uploads the 3D texture, and frees the field once fully dissolved. |
| `BlocksSight(Vector3, Vector3)` | Does this cloud block the line of sight a→b? Integrates density along the segment. |
...
| `SampleDensity(Vector3)` | Density at a world position (nearest cell, deterministic). Returns 0 outside the grid. |
