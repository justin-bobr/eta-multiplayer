...
| `MethodName.PlayGripChange` | Cached name for the 'PlayGripChange' method. |
...
| `PropertyName._gripChangeActive` | Cached name for the '_gripChangeActive' field. |
...
| `ProbeReverbEnv(HitInfo)` | Classifies the gunshot reverb environment via an upward ceiling raycast. Tunnel-tagged ground returns Tunnel, a ceiling hit returns Indoor, otherwise Outdoor. |
...
| `ReplayOneTick(MovementInput)` | Re-simulates one tick with the saved input; physics state is re-derived from current position. Audio/FX/net-send are skipped via `_isReplaying`. |
