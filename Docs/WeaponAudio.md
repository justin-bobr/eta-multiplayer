...
| `PlayDryFire(WeaponStats, Vector3)` | Plays the dry-fire click (empty magazine). Very short 3D hearing range. |
| `PlayReload(WeaponStats, Vector3)` | Plays the reload sound (single layer, dry). Triggered on the reload rising edge. |
| `PlayShoot(WeaponStats, Vector3, ReverbEnv)` | Plays a layered shot (Body + Mech + Tail) with environment reverb; remote players also use the distant clip set and occlusion. |
| `Preload(WeaponStats)` | Pre-loads all clips referenced by a weapon (typical caller: NetworkPlayer._Ready with the starting weapon). |
| `RestoreGodotObjectData(Bridge.GodotSerializationInfo)` | — |
| `ReverbBusFor(ReverbEnv)` | Maps a `ReverbEnv` value to the matching reverb bus name. |
