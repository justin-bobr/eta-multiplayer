...
| `DetectBestEnemy(Vector3, PhysicsDirectSpaceState3D, BotCombatContext)` | Returns the closest hostile, alive, in-range peer with clear LOS; null if none. Skips self, same-team, dead, and unspawned peers. |
...
| `Tick(uint, Vector3, Rid, IReadOnlyList<Vector3>, PhysicsDirectSpaceState3D, uint, BotCombatContext)` | Produces the `InputPacket` for this tick. If a visible enemy is in range, turn toward it and fire once aim is aligned and the reaction delay has elapsed; otherwise wander the NavMesh path, requesting a new path on arrival or when stuck. |
