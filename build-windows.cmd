@echo off
REM ============================================================================
REM  ETA Windows - Release Build.
REM  Single binary used as both client and server:
REM    Export\Windows\eta.exe                              -> client
REM    Export\Windows\eta.exe --headless -- --server       -> dedicated server
REM
REM  Output: D:\Godot\Export\Windows\eta.exe
...
echo.
echo === ETA WINDOWS BUILD ===
echo Godot:    %GODOT%
echo Template: %TEMPLATE%
echo Project:  %~dp0Game
echo Output:   %~dp0Export\Windows\eta.exe
echo.
...
"%GODOT%" --headless --path "Game" --export-release "Windows" "%~dp0Export\Windows\eta.exe"
if errorlevel 1 (
...
REM ----------------------------------------------------------------------------
REM Generate convenience launchers next to eta.exe so the user (or an ops
REM operator) can double-click client.cmd / server.cmd instead of typing
REM CLI args. The actual binary is eta.exe; these are tiny wrappers.
REM   - client.cmd  starts the game in client mode (default).
REM                 Uses --verbose and pipes stdout+stderr to eta.log next to
...
echo @"%%~dp0eta.exe" --headless -- --server --max-players 16>"Export\Windows\server.cmd"

echo.
echo === BUILD OK ===
echo Binary:   %~dp0Export\Windows\eta.exe
echo Client:   %~dp0Export\Windows\client.cmd          ^(double-click^)
echo Server:   %~dp0Export\Windows\server.cmd          ^(double-click^)
for %%F in ("Export\Windows\eta.exe") do echo Size:     %%~zF bytes
echo.
endlocal
