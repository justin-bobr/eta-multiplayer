...
/// TwoBoneIK3D influence).
///
/// Required scene setup under <c>Skeleton3D</c>:
///   - 2x <see cref="Godot.TwoBoneIK3D"/> (IK_L, IK_R) with root/middle/end_bone_name = thigh/calf/foot _l|_r
///   - 2x Marker3D as target (TargetFoot_L/R) and as pole (PoleKnee_L/R) — as siblings
///   - In each TwoBoneIK3D wire settings/0/target_node and settings/0/pole_node
/// Then assign the [Export] fields here in the inspector.
///
/// Server scenes have no mount → no Foot-IK overhead on the server agent (which does not render anyway).
/// </summary>
