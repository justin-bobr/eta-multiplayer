...
/// <summary>Per-tick input for the grenade-charge logic. Server-replayable.</summary>
...
/// <summary>
/// Pure-logic grenade charge. The longer the fire action is held the stronger the throw (charge 0..1).
/// Deterministic: identical input streams produce identical <see cref="ThrownCharge"/> values so client and
/// server produce the same flight path. Kept independent from Godot (like <see cref="MovementController"/>)
/// so the server can replay the same step.
/// </summary>
public class GrenadeController
{
	/// <summary>Tuning reference. Defaults to the global <see cref="ConVars.Sv"/>.</summary>
