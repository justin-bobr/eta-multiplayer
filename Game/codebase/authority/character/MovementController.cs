...
/// <summary>
/// Pure movement logic. Tuning is injected via <see cref="Sv"/> (default <see cref="ConVars.Sv"/>).
/// Deterministic and free of Node3D/physics calls, so it runs inside the server tick and in replay.
/// </summary>
public class MovementController
{
	...
	/// <summary>Sway multiplier for mouse inertia/lean/bob/tilt (all except breathing). 1 = neutral,
	/// &lt;1 during an active hold, &gt;1 during recover.</summary>
	...
	/// <summary>Multiplier for the breathing oscillation only — less damped during a hold than the rest,
	/// amplified during recover.</summary>
	...
	/// <summary>Sub-tick fire-press view yaw, captured at the Fire-bit rising edge inside an event. When
	/// <see cref="SubtickFireViewValid"/>, the fire input uses this instead of the tick-end view so a
	/// mid-flick shot aims at the press moment. Reset to invalid at the top of every <see cref="Step"/>.</summary>
	...
	/// <summary>True on the tick any reload started. One-tick edge; drives the networked mag-drop.</summary>
	...
	/// <summary>Horizontal speed magnitude (X/Z only). Inlined sqrt to avoid a per-call Vector3 ctor in hot paths.</summary>
	...
	/// <summary>Server-replayable movement step. With non-empty <see cref="MovementInput.Events"/>, the tick is
	/// split into segments at each event's TFraction and the inner step runs per segment. Tick-level once-per-step
	/// flags (<see cref="DidJumpThisFrame"/>, <see cref="DidWallJumpThisFrame"/>, <see cref="LastWishDir"/>) are
	/// written here, not in the inner step. Empty Events runs a single legacy segment over the full Dt.</summary>
	...
	/// <summary>One inner physics segment over <see cref="MovementInput.Dt"/> (a tick fraction for subtick
	/// callers, or the full tick legacy). Does not reset the once-per-tick flags managed by <see cref="Step"/>.</summary>
	...
	/// <summary>Crouch-cancel jump: a crouch press inside the narrow apex window grants a small vertical
	/// boost. One-shot per airtime, reset on landing or a new jump.</summary>
	...
	/// <summary>Tracks airborne state from <see cref="MovementInput.OnFloor"/> (deterministic) and lerps the
	/// ADS blend toward its target with the per-weapon blend time.</summary>
	...
	/// <summary>Source/CS-style horizontal movement: ground friction (with a stopspeed floor) then
	/// wish-direction acceleration up to wishspeed; counter-strafe falls out of the addspeed formula. Air
	/// branch uses Quake-style strafe accel independent of ground tuning.</summary>
