...
/// <summary>
/// Foot IK for TPS — Godot 4.6 <c>TwoBoneIK3D</c> plus a ground-snap raycast.
///
/// Minimal version (analogous to the reference demo
/// https://github.com/ownverseplay/Godot4.6.2-foot-ik-demo): performs ONLY ground adaptation, NO
/// procedural stepping. The foot targets are simply snapped per frame to the ground under the
/// respective hip bone. If the legs are already animated (e.g. a Mixamo walk anim), IK adapts the
/// feet to slopes/stairs. Without a walk anim the feet just stand under the hip — no "walking"
/// visual.
///
/// Required scene setup (in smg.tscn under tps_character/root_001/Skeleton3D):
///   1. One TwoBoneIK3D node per leg (e.g. IK_L, IK_R) with:
///        - settings/0/root_bone_name = "thigh_l" / "thigh_r"
///        - settings/0/middle_bone_name = "calf_l" / "calf_r"
...
public class TpsFootIk
{
	...
	/// <summary>Resolves the thigh bones and validates that the wired TwoBoneIK3D refs are present.
	/// Returns false (and disables IK) if anything is missing.</summary>
	...
	/// <summary>Per-tick update: snaps both foot targets to the ground beneath the hips, positions
	/// the knee pole markers, and smooth-lerps the IK influence.</summary>
	...
	// Wiederverwendete Query + Exclude. GroundSnap läuft pro Foot (L/R) pro Frame während IK aktiv ist.
	...
	/// to compensate for the foot bone height above the mesh footprint.</summary>
