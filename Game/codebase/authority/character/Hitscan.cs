...
/// <summary>Hit result of a raycast - server-replayable data.</summary>
...
/// <summary>
/// Pure hitscan logic. Server-replayable: identical FireInput + world-state yields identical
/// HitInfo. The client runs this for visual impacts; the server runs it for damage authority
/// and lag compensation. World access is via SpaceState - on the server the rewound physics
/// world would be used.
/// </summary>
public static class Hitscan
{
	/// <summary>Manual ray-vs-shape cast against a list of (hitbox-node, world-transform, shape) tuples.
	/// Bypassed die Godot Physics-Broadphase komplett — die ist im _PhysicsProcess deferred-updated und
	/// liefert für lag-comp-rewound Hitboxen die ALTEN Positionen → Cast geht durch wo die Hitbox sein
	/// SOLLTE. Dieser manuelle Cast nutzt die rewound GlobalTransforms direkt aus dem Bone-History-Buffer.
	/// Returnt den nächsten Hit der NÄHER ist als <paramref name="maxDist"/> (= world-cast distance).</summary>
	...
	/// <summary>Returns the average of |basis.X|, |basis.Y|, |basis.Z| — used to scale sphere/capsule
	/// radii from the shape's local space to world. Bei tps-character (scale 0.01) ist das ~0.01.</summary>
	private static float AvgScale(Basis b) => (b.X.Length() + b.Y.Length() + b.Z.Length()) / 3f;

	/// <summary>Ray-Sphere Intersection. Standard quadratic. Returns true + t (parametric distance) bei Hit.</summary>
	...
	/// <summary>Ray-Capsule. Capsule = Line-Segment Endpoints A→B mit Radius. Transformiert von Local in
	/// World via <paramref name="xform"/>: Local-Capsule liegt entlang Y-Achse von -(H/2-R) bis +(H/2-R)
	/// (Hemispheren extension um R). Berechnet 3 Subprimitives: Cylinder + 2 Endsphären, takes nearest.</summary>
	private static bool RayCapsule(Vector3 origin, Vector3 dir, Transform3D xform, float radius, float height, out float t)
	{
		// Capsule-Endpoints in WORLD: ±yAxis * (height/2 - radius) vom Center. Wenn height ≤ 2*radius
		// (degeneriert), wird das Sphere-equivalent (Endpoints fallen zusammen am Center).
	...
	/// <summary>Ray-OBB (oriented bounding box). Slab-Test in local space (= ray in box-local frame).</summary>
	private static bool RayBox(Vector3 origin, Vector3 dir, Transform3D xform, Vector3 size, out float t)
	{
		// Transform ray ins OBB-Local-Space.
	...
	// Wiederverwendetes Query-Object + Exclude-Arrays statt new pro Cast() (war per-call alloc von
	...
	/// <summary>Casts a ray from origin in direction for the given range in meters. Optional single-RID exclude (e.g. the shooter).</summary>
	...
	/// <summary>Shared body for <see cref="Cast"/> + <see cref="CastMulti"/>: dispatches the IntersectRay
	/// and copies the result into a <see cref="HitInfo"/>, including per-face + per-group material lookup.</summary>
	...
	/// <summary>
	/// Per-face material lookup. Finds the MeshInstance3D for the collider, derives the surface
	/// (material slot) from face_index, reads the material and checks its "impact_tag" metadata.
	/// Works only for trimesh colliders (ConcavePolygonShape3D, "Create Trimesh Static Body").
	///
	/// Triangle counts per surface are memoized per ArrayMesh (see <see cref="_triCountCache"/>)
	/// because SurfaceGetArrays().AsInt32Array() allocates a new int[] per call.
	/// </summary>
	...
	/// <summary>Returns the triangle count of a surface-array (indexed if present, otherwise vertex-based).</summary>
	...
	// Cache: Collider → MeshInstance3D. FindVisualMesh ist nicht-trivial (GetParent + GetChildren-Iteration
	// pro Aufruf, beides alloziert Godot.Collections.Array<Node>). Cache mit weak-ref-like Behavior via
	// IsInstanceValid-Check on read — wenn Mesh disposed wurde, re-resolve.
	private static readonly System.Collections.Generic.Dictionary<Node3D, MeshInstance3D> _visualMeshCache = new();

	/// <summary>Looks for a MeshInstance3D as a sibling or child of the collider. Best-effort.
	/// Result is cached per collider to avoid repeated GetParent+GetChildren allocations.</summary>
	...
	// Cache: Collider → resolved-material-group. Spart pro Hitscan ein GetGroups()-Alloc (returnt
	// frische Godot.Collections.Array<StringName>) + den linearen HashSet-Scan. Groups ändern sich
	// während eines Match nicht, daher 1-shot Resolve pro Collider safe.
	...
	/// <summary>Per-collider group fallback - the first recognized material group wins. Cached per collider
