...
/// <summary>
/// Editor tool that generates + sizes the per-bone hitbox capsules from a skeleton. Add it under the
/// character root, assign <see cref="Skeleton"/> (optionally a <see cref="HitboxContainer"/> target and
/// per-slot bone overrides), then tick <see cref="Build"/>: it creates BoneAttachment3D → Hitbox →
/// CollisionShape3D per slot, positions the capsule between the bone and its first child, sizes the radius
/// from the mesh, and parents everything to the edited scene so it saves.
///
/// Each *Bone field is a dropdown of the assigned skeleton's bone names — pick the matching bone if your
/// rig uses different names than the defaults. To view the result: Debug → Visible Collision Shapes.
/// </summary>
[Tool, GlobalClass]
public partial class HitboxBaker : Node3D
{
	...
	/// <summary>Where the generated hitbox nodes are placed. Leave empty to create + use a "Hitboxes" node
	/// under the skeleton.</summary>
	[Export]
