...
public partial class NetworkPlayer : CharacterBody3D
{
	...
	// hitbox/aim posing and on puppets, so it is not weapon data. The per-weapon ADS zoom (TpsAimFov)
	...
	/// <summary>ADS FOV is relative to the base FOV: per-weapon AimFov is authored against a 100° base and
	/// scaled by <see cref="HipFov"/>, so the zoom factor stays constant at any FOV setting.</summary>
	...
	// Per-weapon ADS/crouch/canted calibration — moved onto WeaponAnimation; the character reads them
	...
	/// <summary>WeaponBoneModifier on the FPS skeleton applying the ADS/crouch/canted/recoil offset to
	/// ik_hand_gun. Resolved by node path, not a static singleton (which broke the editor ADS preview).</summary>
	[Export]
	...
	protected void UpdateTpsCamera(float dt)
	{
		if (_tpsPivot == null || _tpsCam == null)
			return;
		// tps_pivot is a child of the body (CharacterBody3D), which HandleMouseLook already yaws via RotateY,
		// so the pivot inherits the look yaw and must only add the look pitch locally. Adding _lookYaw here
		// too double-rotated the camera: it faced 2×yaw while movement followed 1×yaw.
	...
	/// <summary>Fires the TPS reload/inspect one-shot on the rising edge of the reload/inspect state. Puppets
	/// read the replicated snapshot flags; the local TPS body reads its own sim.</summary>
	...
	protected void ApplyModeVisibility()
	{
		...
		// TPS skeleton + AnimationTree + aim modifier process every frame for an invisible body, and that cost
		// is engine-side so it never showed in the C# profiler. Re-enabled when spectating through the TPS cam.
		if (local)
		{
	...
	public override void _Ready()
	{
		...
		if (CurrentGameMode != PresentationMode.Server)
		{
			...
			BuildTpsTree();              // TPS body blend tree (wires _tpsActionAnim + Action/Aim additive amounts)
			PreWarmAnimationOneShots(_tree);   // pre-fire the FPS one-shots (fire/reload) so the first shot doesn't hitch
			ApplyViewmodelLayer();
		}
		ApplyModeVisibility();
	...
	/// <summary>[Tool] editor preview of the FPS view. Overridden by <see cref="LocalPlayer"/>; base is a no-op.</summary>
