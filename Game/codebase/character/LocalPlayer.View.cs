...
{
	/// <summary>The local per-frame view chain. Invoked by LocalPlayer._Process after visual interpolation,
	/// and directly in the editor for the [Tool] preview.</summary>
	private void RenderLocalView(double delta)
	{
		if (Engine.IsEditorHint())
		{ ApplyEditorPreview((float)delta); return; }
		if (IsDead)
			return;   // dead = spectating through a puppet cam; hidden viewmodel not animated
	...
	/// <summary>Renders the grenade trajectory preview while the grenade slot is active and fire is held.
	/// Uses <see cref="GrenadeTrajectory.Predict"/> with the same pending-throw origin/velocity the sim uses
	/// on release, so the preview matches the real flight. Built lazily on first show.</summary>
	...
	/// fall past JumpMinFallHeight). Returns whether it counted, so the caller gates the landing sound alike.</summary>
	...
	private void RenderWorldCamera(float dt)
	{
		...
		// Sprint FOV boost: widens FOV while sprinting (eased via smoothstep) and drives the peripheral
		// sprint-blur overlay from the same eased value. FovBoost/FovBlendSpeed live in ConVars.Cl.
	...
	/// <summary>Lazily builds the peripheral sprint-blur overlay: a full-screen ColorRect running
	/// sprint_blur.gdshader on a CanvasLayer behind the viewmodel (layer -1, so the weapon/HUD stay crisp).</summary>
	...
	/// <summary>Fades in ADS depth-of-field: world camera focuses far via CameraAttributes DOF, weapon blurred
	/// by the 2D viewmodel_ads_blur shader (CameraAttributes DOF doesn't render in the weapon's transparent_bg
	/// SubViewport). Also feeds AdsBlend into the screen-space post-FX.</summary>
	...
	/// <summary>Drives the viewmodel_ads_blur shader on the weapon SubViewportContainer — a 2D pseudo-DOF
	/// keeping the iron-sight zone sharp. The only way to blur the weapon, since CameraAttributes DOF doesn't
	/// render in its transparent_bg SubViewport.</summary>
	...
	/// <summary>World far-DOF for ADS. Far DOF stays enabled; only the amount fades with ADS (no per-frame toggle).</summary>
	...
	protected override void ApplyEditorPreview(float dt = 0f)
	{
		...
		// so the WeaponBoneModifier (ADS offset) never runs and live offset edits don't show.
