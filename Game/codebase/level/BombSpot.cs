...
///
/// Two roles:
///   1. **HUD navigation**: <see cref="HudCs2"/> resolves <see cref="Level.BombSpotForSlot"/> for
...
/// (the mapper picks A / B / C in the inspector) and the map's <see cref="Level"/> registry.
/// Lookups go via <see cref="Level.BombSpotForSlot"/>, not <see cref="SceneTree.GetNodesInGroup"/>.
///
...
public partial class BombSpot : Zone
{
	...
	/// letter the compass diamond + UI label use for this spot. The map's <see cref="Level"/>
	/// registry holds the BombSpot paths so the runtime side resolves a slot to its BombSpot without
	/// any group plumbing.</summary>
