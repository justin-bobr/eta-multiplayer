...
/// <summary>
/// CS-style team-select overlay shown while the player is in <see cref="Team.Spectator"/> during the
/// competitive-mode handshake. Two big buttons (Team1/Team2) plus a roster column on each side
/// listing the players currently on that team — pulled live from <see cref="NetClient.RemotePlayers"/>.
/// The background is a full-screen back-buffer blur shader so the preview-camera view stays visible
/// but defocused, matching the CS2 main-menu look.
///
/// Clicking either side fires <see cref="PacketType.TeamSelect"/>. The menu auto-removes when
/// <see cref="NetClient.SpawnAuthorized"/> flips true (= server granted the spawn).
/// </summary>
public partial class TeamSelectionMenu : CanvasLayer
{
	...
	public override void _Ready()
	{
		...
		// Subscribe to NetClient join/left events for instant roster updates instead of waiting
		// for the 2s polling tick. Polling still runs as a safety net (in case an event is missed
		// during the brief window between menu construction and event subscription).
	...
	private void BuildUi()
	{
		...
		// Small "Spectate" button below the two team buttons — keeps the user in preview-cam
		...
		// Spectator section sits below the team-buttons in the centre column. Hidden when empty so
		// the layout doesn't reserve a gap. Header label tracks the count, list shows centred names.
	...
	/// <summary>Builds one side's roster column — header + scrollable VBox for player entries. Returns the inner VBox so the caller can populate it dynamically while keeping the styled outer wrapper.</summary>
	...
	/// <summary>Walks NetClient.RemotePlayers and rebuilds Team1/Team2/Spectator lists. Triggered by
	/// OnPlayerJoined/OnPlayerLeft events for instant updates plus a 2s polling fallback. Cheap —
	/// the rosters are tiny (max 16 players typically).</summary>
	...
	private void AddRosterEntry(VBoxContainer list, string playerName, byte netId, bool centered)
	{
		...
		// Tiny color square as a stand-in for a future avatar; netId-derived so it's deterministic.
	...
	/// <summary>"Nur zuschauen": hides the menu without sending a TeamSelect packet — the user
	/// stays in Spectator and the PreviewCameraController keeps cycling cams. Mouse mode is
	/// captured so the user can look around with the cycled preview cams. LocalPlayer never
	/// spawns until/unless the user re-opens the team-select UI (future key-bind).</summary>
...
/// <summary>Quality-of-life helper — removes all children of a container in one call. Used by the team-selection menu to rebuild its roster lists on every refresh without holding onto stale handles.</summary>
