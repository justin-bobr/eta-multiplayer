...
/// Fixed <see cref="FixedDt"/>, raycast-only, no RigidBody/randomness: same inputs land identically on every client.
/// </summary>
public static class GrenadeTrajectory
{
	public const float BaseGravity = 17.5f;
	/// <summary>Effective gravity (range knob), set by NetworkPlayer from GrenadeRangeScale; smaller = travels farther.</summary>
	...
	/// <summary>One projectile step (gravity, raycast move with bounce, ground drag). Returns whether grounded after the step.</summary>
	...
	/// <summary>Simulates from origin/vel until rest (or <see cref="MaxFlyTime"/>), filling <paramref name="pathOut"/> with world-space points and the landing point.</summary>
