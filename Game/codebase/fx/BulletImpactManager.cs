...
/// <summary>
/// Bullet impact effect manager. Spawns a Decal3D (bullet hole) and GPUParticles3D (sparks/dust)
/// at the hit point. The surface material is identified via a Godot group on the collider
/// (e.g. AddToGroup("metal")). If no decal texture is configured for a material the decal step
/// is skipped and only particles are emitted.
/// </summary>
public partial class BulletImpactManager : Node3D
{
	...
	/// <summary>
	/// Diagnostic: logs the CPU time of decal and particle spawning per impact. If timings are
	/// fast but the game still hitches the cause is GPU shader compilation on first render,
	/// not this code.
	/// </summary>
	...
	/// <summary>
	/// Distance fade hides depth-precision flicker on far decal projections and saves render cost.
	/// </summary>
	...
	/// <summary>Registers the singleton, starts the async decal prewarm and pre-allocates the
	/// Decal + Particle pools (deferred so SceneTree.Root is reliably reachable). Spart sonst
	/// einen 5-30ms Hitch beim ersten Schuss wenn der Pool lazy initialisiert würde.</summary>
	...
	/// <summary>
	/// Warms all decal sets on a background thread at level load so the expensive texture packing
	/// (per-pixel ORM pack and Albedo/Opacity merge) does not run on the main thread on the first
	/// shot. Until the prewarm finishes the lazy path takes over (thread-safe via lock).
	/// </summary>
	...
	/// <summary>Clears the singleton reference when this node leaves the scene tree. Pool-Nodes
	/// hängen als Children der SceneTree.Root (= Window, persists across scenes), daher manuell
	/// freen — sonst leaks bei jedem Scene-Reload (z.B. Disconnect+Reconnect).</summary>
	...
	private const float RecycleInterval = 0.25f; // 4Hz reicht — Lifetimes sind >= 0.6s

	/// <summary>Periodisches Aufräumen abgelaufener Pool-Slots — versteckt Decals + stoppt Particles
	/// die ihre Expiry-Time überschritten haben. Vorher: per-Slot SceneTreeTimer + Closure-Delegate
	...
	/// <summary>Spawns an impact: decal (when a texture is available) plus material-coded particles.</summary>
	...
	/// <summary>
	/// Random-pick from a pool with a null filter. If the chosen element is invalid the others
	/// are tried in circular order. Returns null when the whole pool is empty or invalid.
	/// </summary>
	...
	// Pre-allokierter Pool — vorher: per-Schuss new Decal + AddChild + new SceneTreeTimer + Closure-Delegate.
	// Jetzt: round-robin durch fixe Anzahl Decals, overwrite oldest wenn voll. Despawn via per-slot expiry
	// time + Hide statt QueueFree.
	...
	/// <summary>Spawns the decal node for the given hit using the material-specific pool with default fallback.</summary>
	...
	/// <summary>Cached, reusable rendering resources for one particle material profile.</summary>
	...
	/// <summary>Returns the cached material/mesh bundle for a material key, building it on first request.</summary>
	...
	// Particle-Pool — pre-allokierte GpuParticles3D + ProcessMaterial-Instances. Per Spawn nur die
	// per-shot-Properties (Direction, Color, Spread, Speed, Gravity) auf der vorhandenen ProcessMaterial
	// mutaten + Restart() — kein new GpuParticles3D + new ParticleProcessMaterial pro Schuss mehr.
	...
	/// <summary>Spawns the GPU particle burst configured by the material's <see cref="ParticleProfile"/>.</summary>
	...
	/// <summary>Returns the particle profile (color, count, gravity, etc.) for a given material tag.</summary>
	...
	/// <summary>Builds an orthonormal basis where local-Y points along the surface normal (decals project along -Y).</summary>
