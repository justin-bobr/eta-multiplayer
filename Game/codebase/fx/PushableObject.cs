...
/// <summary>
/// Pushable physics object (e.g. a car). The player stands behind it, holds the
/// push action, and presses against the body; after enough sustained pressure
/// (<see cref="MinChargeSeconds"/>) a force is applied each physics tick along
/// the preconfigured <see cref="PushDirection"/>. Heavier bodies move slower.
/// The script lives on a <see cref="RigidBody3D"/> root so gravity, ground
/// contact and tipping come for free. A movement is recognised only when the
/// player both holds the action and produces a slide collision against this
/// body whose direction matches <see cref="PushDirection"/>. A 2D hint appears
/// when the player is close enough behind the object. In editor mode ([Tool])
/// a yellow arrow visualises the push direction; the arrow is editor-only and
/// is not saved to the scene. Currently client-local; networked behaviour
/// would require the server to apply the force.
/// </summary>
[Tool]
public partial class PushableObject : RigidBody3D
{
	...
	/// <summary>Caches the spawn position for the max-push-distance limit; builds the editor gizmo on scene open.</summary>
	...
	/// <summary>Editor-only: keeps the direction arrow in sync with the live PushDirection value.</summary>
	...
	// Aktivitäts-Range — wenn der Spieler weiter weg ist, skipt _PhysicsProcess komplett.
	// Spart Cost auf idle Pushables (ResolvePlayer + GetSlideCollisionCount + Vector-Math).
	// 5m ist deutlich mehr als der Push-Range (typisch 1-2m).
	...
	public override void _PhysicsProcess(double delta)
	{
		...
		// Sleep-Gate: wenn Spieler weiter weg als ActiveDistance + Pushable idle (charge==0, sleeping=true,
		// kein wasMoving) → skip vollständig. Re-check nur alle 8 Ticks (~62.5ms bei 128Hz) damit der
		// Distance-Check selbst nicht teurer wird als das was er einsparen soll.
	...
	/// <summary>Resolves the key/button bound to <see cref="PushAction"/> in the Input Map for the on-screen prompt.</summary>
	...
	/// <summary>
	/// Editor visual: draws a yellow arrow along <see cref="PushDirection"/> with length
	/// <see cref="MaxPushDistance"/>. The arrow node has no owner, so it is not saved to the scene.
	/// </summary>
