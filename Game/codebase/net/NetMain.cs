...
/// <see cref="NetworkPlayer._PhysicsProcess"/>.</summary>
public partial class NetMain : Node
{
	...
	/// <summary>Returns <see cref="LocalPlayer"/> for other systems (Crosshair, DebugOverlay, reconcile).</summary>
	...
	/// a Client without <c>--connect</c> waits for the menu to call <see cref="ConnectToServer"/>.</summary>
	...
	/// <summary>Main-menu Connect button: applies host/port, starts the client, switches to the loading scene.</summary>
	...
	/// <summary>Creates a NetClient, wires its event subscribers, and starts it.</summary>
	...
	/// <summary>One-shot: on a Spectator SpawnAck (competitive, no spawn yet), spawns the PreviewCameraController
	/// and TeamSelectionMenu. Both self-destruct once SpawnAuthorize arrives. Skipped for deathmatch.</summary>
	...
	/// <summary>Instantiates the LocalPlayer + HUD into the Players container once SpawnAck has arrived,
	/// the spawn is authorized, and world.tscn is active.</summary>
	...
	/// <summary>GPU frame time (ms): sum of measured render times across all tracked viewports. 0 until measurement is enabled.</summary>
	...
	/// <summary>Render-thread CPU time (ms) summed over all tracked viewports. Companion to <see cref="MeasuredGpuMs"/>.</summary>
	...
	/// <summary>Logs frame spikes above <see cref="SpikeThresholdSec"/> with GC stats and Godot perf deltas.
	/// Gated on Dbg.Enabled so production builds carry no overhead.</summary>
	...
	/// transport drop. Unsubscribes from <see cref="NetClient.OnDisconnected"/> first so cleanup runs once.</summary>
	...
	/// <summary>True in the post-disconnect idle state (set on disconnect, cleared on Reconnect/Quit).
	/// SceneLoader checks this to suppress its auto-connect logic.</summary>
	...
	/// <summary><see cref="DisconnectScreen"/> reconnect button: tears down the old client, starts a fresh one,
	/// and re-enters loading.tscn to rerun the connect flow.</summary>
