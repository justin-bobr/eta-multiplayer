...
/// <summary>
/// Server-side line-of-sight visibility precomputation. Riot Valorant style: the map is voxelised
/// into a coarse 3D grid (default 4m cells, auto-coarsened to stay under a per-path voxel cap when
/// the world AABB is large), and pairwise visibility between every voxel pair is determined via a
/// single center-to-center physics raycast. At runtime <see cref="CanSee"/> is an O(1) bit lookup —
/// cheap enough to gate every snapshot and every position-leaking event broadcast.
///
/// Build is INCREMENTAL: <see cref="BeginBuild"/> sets up the grid, then <see cref="StepBuild"/>
/// is called repeatedly with a small raycast budget so neither the server tick nor the editor frame
/// blocks. <see cref="Built"/> only flips true when the last pair completes — until then
/// <see cref="CanSee"/> returns true (no culling) so the game keeps playing with the old behavior.
///
/// Storage is a flat <see cref="byte"/>[] of ceil(N² / 8) bytes — the SAME format the baked
/// <see cref="VoxelPvsData"/> resource serialises to disk. <see cref="LoadFromData"/> can therefore
/// adopt the resource's buffer by reference, with zero copy and zero memory duplication.
///
/// Visibility is OPTIMISTIC: a single center-ray is "pessimistic" (misses partial visibility around
/// corners), so we accept some over-occlusion. FoW must never accidentally hide a visible enemy,
/// but it MAY occasionally reveal one that's just barely visible.
/// </summary>
