...
/// <summary>
/// Spawn-marker management for round-based modes (CT vs T) and Deathmatch. On map load it reads the
/// active map's <see cref="Level"/> registry (via <see cref="World.Level"/>) and buckets every
/// <see cref="Spawn"/> by its <see cref="Spawn.SpawnKind"/> — no group lookups, no tree walk.
///
/// Mapper convention:
///   1. In the Godot editor, add a <see cref="Spawn"/> node to the map and set its <see cref="Spawn.Kind"/>
///      (Team1 / Team2 / Deathmatch) and Size in the inspector.
///   2. Add its path to the map root's <see cref="Level.SpawnPaths"/> (or hit Level's "Collect Children").
///   3. Provide at least 4 to 10 spawns per team to avoid over-dense spawns.
///
/// Fallback: when the map has no spawns at all, a hard-coded default position (mid-map) is used.
/// </summary>
