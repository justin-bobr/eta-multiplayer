...
/// back-buffer. Clicking a side sends <see cref="PacketType.TeamSelect"/>; auto-removes when
/// <see cref="NetClient.SpawnAuthorized"/> flips true.
/// </summary>
public partial class TeamSelectionMenu : CanvasLayer
{
	...
	private void BuildUi()
	{
		...
		// mode without spawning a LocalPlayer. Used for join-as-spectator.
	...
	private void RefreshRosters()
	{
		...
		if (client != null)
		{
			...
			// Add the local player entry; it is not in RemotePlayers (which holds only remote players).
			// Shown as Spectator since the local user is in the team-select phase.
