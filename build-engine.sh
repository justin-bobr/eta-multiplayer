#!/usr/bin/env bash
# =============================================================================
#  Builds the custom Linux export template via scons inside WSL.
#  Invoked by build-engine.cmd after the Windows template build succeeds.
#  Can also be run directly on a Linux host:
#    cd /mnt/d/Godot && bash build-engine.sh
#
#  Output: Engine/Templates/linux_release.x86_64
# =============================================================================
set -euo pipefail

# Resolve script directory in a WSL-friendly way ("D:\Godot" -> "/mnt/d/Godot")
...
# ---------------------------------------------------------------------------
# apt deps required by Godot's linuxbsd build per
# https://docs.godotengine.org/en/stable/contributing/development/compiling/compiling_for_linuxbsd.html
# Plus scons (via apt or pip). dotnet-sdk-8.0 is needed for the mono module.
# ---------------------------------------------------------------------------
...
# dotnet-sdk-8.0 - try apt first, fall back to Microsoft repo if missing.
...
# ---------------------------------------------------------------------------
# Source tree should exist - build-engine.cmd's git clone runs first.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Mono glue + managed assemblies must exist before building a mono-enabled
# template. We reuse the stock Linux editor binary in Engine/ (downloaded
# during setup-editor on the Windows side, or installed manually on Linux)
# to run --generate-mono-glue, then build_assemblies.py for the managed code.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# scons build with the same module strip as build-engine.cmd's Windows build.
# Linux-specific notes:
#   - debug_symbols=no               smaller binary, no debug info
#   - use_static_cpp=yes             portable libstdc++ (runs on older Linux)
#   - lto=full                       link-time optimisation
# ---------------------------------------------------------------------------
cd "$GODOT_SRC"
echo
...
# ---------------------------------------------------------------------------
# Copy result. Mono builds produce a .mono variant; pick the produced one.
# Plus the GodotSharp managed runtime folder is required next to the template.
# ---------------------------------------------------------------------------
